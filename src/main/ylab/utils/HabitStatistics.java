...
public class HabitStatistics {
    ...
    public static long getCompletionCountForWeek(Habit habit, LocalDate weekStart) {
        ...
        return completionHistory.stream()
                .filter(completionDate -> !completionDate.isBefore(weekStart) && completionDate.isBefore(weekEnd))
                .count();
    }
    ...
    public static int getCurrentStreak(Habit habit) {
        ...
        for (int i = completionHistory.size() - 1; i >= 0; i--) {
            LocalDate completionDate = completionHistory.get(i);

            long daysBetween = ChronoUnit.DAYS.between(completionDate, today);
